/**
 * Created by uthornbl on 2013-11-06.
 *
 * NB: Requires jQuery
 *
 */
...
$(document).ready(function () {
    // Load scripts we depend on and hand over to init1()
    // $.getScript("common/Silverlight.js")
...
function checkHiveInstalled() {
    // Check if we have Hive installed and pass control over to init2
    detectHiveClient(function (version) {
        if (version !== "")
            hiveInstalled = true;
        processPlayer(); // Do the real work
    });
}
...
function processPlayer() {
    ...
    $(".hive-player").each(function (index) {
        ...
        // log(json.streams[0].manifest);

        var ok = true;
        var dg = elem.attr("data-player-draggable");
        if (dg == "true")
            elem.draggable({
                cursor: "move",
        ...
        // Change port based on protocol
        var clientPath = "http://127.0.0.1:8999/"
        if (location.protocol === 'https:')
            clientPath = "https://127.0.0.1:9000/"

        // Define Hive installation path.
        var installPath = (elem.attr("data-installer") !== undefined) ? elem.attr("data-installer") :
          "http://install.hivestreaming.com/?show-test-info=1&directory=test";
                ...
                if (hiveInstalled) {                   
                    if ((mf.search("api-test") >= 0) || (mf.search("customer-callbacks") >= 0) ||
                      (mf.search("/v1/") >= 0))
                        ...
                        switch (streamType) {
                            case "hds":
                                mf = clientPath + "hds/manifest.f4m?callback=" + mf;
                                break;
                            default:
                                // Smooth is the default
                                mf = clientPath + "smoothstream?callback=" + mf;
                                break;
                        }
                    else
                        // Simple URL
                        switch (streamType) {
                            case "hds":
                                mf = clientPath + "hds/manifest.f4m?url=" + mf;
                                break;
                            default:
                                // Smooth is the default
                                mf = clientPath + "smoothstream?url=" + mf;
                                break;
                        }
                } else {
...
function detectHiveClient(callback) {
    // Change port based on protocol
    var clientVersion = "http://127.0.0.1:8899/version.json"
    if (location.protocol === 'https:')
        clientVersion = "https://127.0.0.1:8900/version.json"

    $.support.cors = true;
    $.getJSON(clientVersion)
      .done(function (data) {
          if (callback)
      ...
      .fail(function (jqxhr_1, textStatus_1, err_1) {
          if (window.location.protocol.indexOf("https") > -1) {
              ...
              if (callback)
                  callback(""); // Return an empty string, no hive installed
          }
      });
}
